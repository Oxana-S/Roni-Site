...
### ✅ Сделано (аудит 2026-06-28)
- **Мёртвый код старой шапки** убран с 9 страниц (`onScroll`/слушатель/`componentWillUnmount`/
  бинд + дубль `.vhead` CSS). `scrolled`/`headScrolled`/`langBtns` оставлены инертными.
- **`404.html`/`dev.html`** мигрированы на `<dc-import name="SiteHeader">` (полное меню).
- **`document.documentElement.lang`** перенесён из `renderVals()` в `componentDidMount`/
  `componentDidUpdate` (`syncLang()`) в SiteHeader.
- **Золотая альфа** централизована: `--gold-rgb:201,168,106` + замена 145 `rgba(201,168,106,α)`
  → `rgba(var(--gold-rgb),α)`. Смена базы золота — теперь в одном месте.

### ⏳ Осталось (не критично)
- 🟡 **Аудио** `assets/audio/chapter-1.mp3` отсутствует — плеер на главной даст 404 при Play.
  Нужен файл записи; пока считать «контент в ожидании» (как часть картинок).
...
- 🟡 **Шрифты Google Fonts** грузятся ~9 разными URL — унифицировать в один (минорно, кэшируется).
- 🟡 Остаток хардкода палитры (`#a89c84`, `#f0e6d2`, тёмные поверхности) вне токенов — при желании
  расширить `:root` (см. аудит дизайн-системы).
